...
impl<'ctxt> Parser<'ctxt> {
    ...
    fn matches_closure(&mut self) -> bool {
        ...
        loop {
            ...
            match self.next_type(lookahead) {
                Some(TokenType::Identifier(_)) => match self.next_type(lookahead + 1) {
    ...
    fn matches(&self, token_type: TokenType) -> bool {
        self.current().filter(|tt| *tt == token_type).is_some()
    }
...
mod tests {
    ...
    pub fn invalid_use_stmts() {
        ...
        let errors: Vec<Diagnostic> = result.1.filter(DiagnosticKind::Error).collect();
